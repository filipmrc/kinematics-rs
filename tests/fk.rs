...
fn test_fk() {
    ...
    let xf_gt = dmatrix![-0.30469028,  0.70209945,  0.64359941,  0.16125383;
                        -0.01531924,  0.67202942, -0.74036598,-1.06391064;
                        -0.95232828, -0.23544177, -0.1940052,  -0.03745314;
                        0.0, 0.0, 0.0, 1.0];
    let robot = Chain::new(String::from("models/ur10_mod.urdf"), String::from("world_joint"));
    assert!((xf_gt - robot.fk(&config, &String::from("ee_fixed_joint"))).abs().sum() < 1.0e-6);
    ...
    let xf_gt = dmatrix![ 0.43628054,  0.33805164,  0.8338947,   0.59856079;
...
fn test_jacobian() {
    ...
    // UR10
    let config = vec![-1.6333352864886936, 1.9904832183698629, -1.058632445175561, 0.9097905013220551, 0.8112221073340007, -2.0020893277255416];
    let jac_gt = dmatrix![0.0,  9.98045077e-01,  9.98045077e-01, 9.98045077e-01,  1.67211236e-02,  6.43599407e-01;
    ...
    let robot = Chain::new(String::from("models/ur10_mod.urdf"), String::from("world_joint"));
    assert!((jac_gt - robot.jacobian(&config, &String::from("ee_fixed_joint"))).abs().sum() < 1.0e-6);

    // KUKA IIWR
    let config = vec![-0.5770057653280771, 0.8797884979496464, -0.6500974986020784, -0.04409327129573626, 2.270870130215024, 0.8326346174333068, -2.44789448732016];
    let jac_gt = dmatrix![0., 0.5455169184296017, 0.6458431523016438, -0.7575363936562216, 0.6494029826015326, -0.5388746630729301, 0.8338947036306463;
